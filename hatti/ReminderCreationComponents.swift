...
struct ReminderLocation {
    ...
    var isValid: Bool {
        !displayName.isEmpty && CLLocationCoordinate2DIsValid(coordinate)
    }
...
enum ConditionType: String, CaseIterable {
    ...
    var displayName: String {
        switch self {
        case .exact: return "Exact Temperature"
        case .range: return "Temperature Range"
        case .trend: return "Temperature Trend"
        case .seasonal: return "Seasonal Marker"
        }
    }
    
    var description: String {
        switch self {
        case .exact: return "Trigger at a specific temperature"
        case .range: return "Trigger within a temperature range"
        case .trend: return "Trigger based on temperature trends"
        case .seasonal: return "Trigger on seasonal transitions"
        }
    }
    
    var icon: String {
        switch self {
        case .exact: return "thermometer"
        case .range: return "thermometer.medium"
...
enum ActivityCategory: String, CaseIterable {
    ...
    var displayName: String {
        switch self {
        case .none: return "Select Category"
...
struct RangeSlider: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                Rectangle()
                    .fill(Color.blue)
                    .frame(width: activeRangeWidth(geometry: geometry), height: 4)
                    .cornerRadius(2)
                    .offset(x: lowerBoundOffset(geometry: geometry))
                
                // Lower bound handle
                Circle()
                    .fill(Color.blue)
                    .frame(width: 20, height: 20)
                    .offset(x: lowerBoundOffset(geometry: geometry) - 10)
                    .gesture(
                        DragGesture()
                            .onChanged { value in
                                updateLowerBound(value: value, geometry: geometry)
                            }
                    )
                ...
                Circle()
                    .fill(Color.blue)
                    .frame(width: 20, height: 20)
                    .offset(x: upperBoundOffset(geometry: geometry) - 10)
                    .gesture(
                        DragGesture()
                            .onChanged { value in
                                updateUpperBound(value: value, geometry: geometry)
                            }
                    )
            }
        }
    ...
    private func lowerBoundOffset(geometry: GeometryProxy) -> CGFloat {
    ...
    private func upperBoundOffset(geometry: GeometryProxy) -> CGFloat {
    ...
    private func activeRangeWidth(geometry: GeometryProxy) -> CGFloat {
        let percentage = (range.upperBound - range.lowerBound) / (bounds.upperBound - bounds.lowerBound)
        return CGFloat(percentage) * geometry.size.width
    }
    
    private func updateLowerBound(value: DragGesture.Value, geometry: GeometryProxy) {
        let percentage = Double(value.location.x / geometry.size.width)
        let newValue = bounds.lowerBound + percentage * (bounds.upperBound - bounds.lowerBound)
        let snappedValue = round(newValue / step) * step
        let clampedValue = max(bounds.lowerBound, min(range.upperBound - step, snappedValue))
        range = clampedValue...range.upperBound
    }
    
    private func updateUpperBound(value: DragGesture.Value, geometry: GeometryProxy) {
        let percentage = Double(value.location.x / geometry.size.width)
        let newValue = bounds.lowerBound + percentage * (bounds.upperBound - bounds.lowerBound)
        let snappedValue = round(newValue / step) * step
        let clampedValue = max(range.lowerBound + step, min(bounds.upperBound, snappedValue))
