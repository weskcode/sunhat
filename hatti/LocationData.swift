...
final class LocationData {
    ...
    // Relationships
    @Relationship(deleteRule: .cascade, inverse: \WeatherReminder.location)
    var weatherReminders: [WeatherReminder] = []
    
    @Relationship(deleteRule: .cascade, inverse: \WeatherData.location)
    ...
    convenience init(from clLocation: CLLocation, placemark: CLPlacemark? = nil) {
        ...
        if let placemark = placemark {
            ...
            self.displayName = [placemark.locality, placemark.administrativeArea]
                .compactMap { $0 }
                .joined(separator: ", ")
        }
        
        if let timeZone = clLocation.timestamp.timeZone ?? TimeZone.current {
            self.timeZoneIdentifier = timeZone.identifier
            self.timeZoneAbbreviation = timeZone.abbreviation() ?? ""
            self.utcOffset = timeZone.secondsFromGMT()
        }
    }
}
