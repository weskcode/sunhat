...
from the DARK-mode raw captures in RawDark/. Same shot list/crops/captions as the
light-mode set (generate_screenshots_v2.py) - only the source screenshots and
output directory differ, since HIG best practice is to showcase both appearances."""
...
RAW_DIR = os.path.join(BASE, "RawDark")
OUT_DIR = os.path.join(BASE, "FramedDark")
...
if __name__ == "__main__":
    # Same shot list, crops, and captions as the light-mode set - only the
    # source raw screenshots differ (dark-appearance captures).
    shots = [
        {
    ...
    print(f"Generating {len(shots)} DARK-mode screenshots at {W}x{H}px...\n")
    for s in shots:
        generate(s)
